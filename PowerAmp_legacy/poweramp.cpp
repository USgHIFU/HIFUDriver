...
void PowerAmp::initialize()
{
    ...
    //  TODO: Remove the name of previous serial port from the list
    for (int i=1;i<DEV_TEST_COUNT;i++)
    {
        setPort();
    }
...
QByteArray PowerAmp::computeBaVolt(ACTION action, VOLT volt)
{
    ...
    case START:
        ...
        {
            int iVolt = (int)ceil(validateVolt(volt) * 10);
            baVolt.resize(2);
            baVolt[0] = 0x40 + iVolt / 128;
            baVolt[1] = iVolt % 128;
        }
        break;
    case ECHO_VOLT:
...
bool PowerAmp::startAll(VOLT volt)
{
    ...
    qCDebug(PA()) << PA().categoryName()
                  << "startAll Time: "<< (timeEnd - timeStart) / double(1000) << "s";
...
bool PowerAmp::startAll2(VOLT volt)
{
    ...
    double timeEnd = (double)clock();
    qCDebug(PA()) << PA().categoryName()
                  << "startAll2 Time: "<< (timeEnd - timeStart) / double(1000) << "s";
...
bool PowerAmp::resetAll()
{
    ...
    double timeEnd = (double)clock();
    qCWarning(PA()) << PA().categoryName()
                    << "resetAll Time: "<< (timeEnd - time_Start) / double(1000) << "s";
...
bool PowerAmp::resetAll2()
{
    ...
    double timeEnd = (double)clock();
    qCWarning(PA()) << PA().categoryName()
                    << "resetAll2 Time: "<< (timeEnd - timeStart) / double(1000) << "s";
